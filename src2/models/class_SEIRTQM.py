...
"""
SEIRTQM Model Implementation
SEIR + Tested + Quarantined + Immune (vaccines)

"""

class SEIRTQM:  
    def __init__(self,tsim,beta,alpha=1,mu=1.4,sigma = 0.2, gamma = 0.1, k=0,I0=100,I_ac0=0,I_d0=0,R0=0,population=1000000,expinfection = 0, SeroPrevFactor=1,chi = 0,psi = 0,k_I=0,k_R=0,RealIC=None, initdate = None,SimIC=None,I_det_prop=1,testaccuracy=1,lambda_Q=1,T_T = 1,T_Q = 14,lambda_Tr=0,vac_d=0,imm_loss=0,imm_gain=0,M0 = 0,S_v0 = 0):
        self.tsim = tsim        
        self.beta = beta
        self.mu=mu
        self.k_I = k_I
        ...
        if type(chi) == int or type(chi) == float:
        ...
        # Vaccines
        if type(vac_d) == int or type(vac_d) == float:
            vac_d = np.poly1d(vac_d)
        ...
        self.vac_d = vac_d
        #self.spare_vac = spare_vac

        self.imm_loss = imm_loss
        self.imm_gain = imm_gain
        self.M = M0
        self.S_v = S_v0
        ...
        # Susceptibles
        # dS/dt:
        self.dS=lambda t,S,E,I,R,I_T,M: self.chi(t) -self.alpha(t)*self.beta*S*(self.expinfection*(E)+I+I_T)/(self.N+self.k_I*(I+I_T)+self.k_R*R)+self.eta*R - self.vac_d(t) + self.imm_loss*self.M
        # Exposed
        # dE/dt
        ...
        if self.Noimm_gain:            
            self.dM =  lambda t,M,S_v: self.vac_d(t) - self.imm_loss*self.M
            self.dS_v =  lambda t,S_v: 0
        else:            
            self.dM =  lambda t,M,S_v: self.imm_gain*self.S_v - self.imm_loss*self.M
            self.dS_v =  lambda t,S_v: self.vac_d(t) - self.imm_gain*self.S_v
    ...
    def integr(self,t0=0,T=500,h=0.01,E0init=False):
        ...
        def model_SEIR_graph(t,y,ydot):
            ...
            ydot[0]=self.dS(t,y[0],y[1],y[2],y[3],y[8],y[10])
            ...
            ydot[10]=self.dM(t,y[10],y[11])
            ydot[11]=self.dS_v(t,y[11])


            
        initcond = np.array([S0,E0,I0,R0,I_ac0,I_d0,e0,e_I0,I_T0,Q0,M0,S_v0])
        ...
        self.M=sol.values.y[:,10]
        self.S_v=sol.values.y[:,11]
